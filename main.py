...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_1x1_layer4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same',
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
				kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output_layer4 = tf.add(output_layer7, conv_1x1_layer4)
    input_layer3 = tf.layers.conv2d_transpose(output_layer4, num_classes, 4, strides= (2, 2), padding='same', 
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
				kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    conv_1x1_layer3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same',
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
				kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output_layer3 = tf.add(input_layer3, conv_1x1_layer3)
    output = tf.layers.conv2d_transpose(output_layer3, num_classes, 16, strides= (8, 8) , padding='same', 
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
				kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    return output
    ...
    for epoch in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            _, loss = sess.run([train_op, cross_entropy_loss], feed_dict={input_image: image,
                                          correct_label: label, keep_prob: 0.5, learning_rate:0.0009})
...
def run():
    ...
    with tf.Session() as sess:
        ...
        epochs = 50
        batch_size = 10 #GPU cannot handle larger batch size
